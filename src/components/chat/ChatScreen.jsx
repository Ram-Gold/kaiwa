...
export default function ChatScreen({ settings, personaId, onBackToDashboard }) {
  ...
  async function submitMessage(messageText) {
    ...
    try {
      const reply = await sendMessage(settings, persona, historyForApi, cleanMessage);
      setMessages((current) => [
        ...current,
      ...
      setMessages((current) => [
        ...current,
        {
          ...
          content:
            error?.userMessage ||
            'Something went wrong while contacting the AI service. Try again.',
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  async function translateAiMessage(message) {
    ...
    try {
      const english = await translateMessage(settings, message.content);
